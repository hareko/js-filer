 ...
 * 
 * @package     Filer
 * @author      Vallo Reima <vallo@vregistry.com>
 * @license     GNU LGPL http://opensource.org/licenses/lgpl-3.0.html
 * @copyright   (c) 2014,2017 Hareko
 * 
 * @param {object} $ jQuery
...
//    nameValidate: /^[^\\\/?%*:|'\"<>]+$/, /* Windows-like test pattern */
    ...
    var change = ['canDrag', 'rootLeaf', 'knotRemove', 'userAuth', 'urlAjax']; /* dynamically changeable */
    var txts = {
...
//          i = $(e.target).jstree(true).settings.types[0].icon;
                    r = $(e.target).children('ul');
                    if (r.length) {
                        r = $(r[0]).children('li');//04.2017
                        for (j = 0; j < r.length; j++) {  /* root icon/type adjustment */
...
//            $(e.target).jstree('set_icon', r[j], i); // replace default root icon
                    ...
                    if (data.node && $(tree.Id).find('.jstree-clicked').length === 1) {
                        if ($.jsfiler.Engaged()) {  /* prevent selection */
                            ...
                            if (tree.selectOpen == 1 || tree.selectOpen == 3) {//07.2017
